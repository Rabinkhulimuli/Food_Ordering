...
export default function OrderBook({ restaurant, cartItem ,removeCart,subFromCart}: Prop) {
    const getTotalCost = ()=> {
        const totalInPence = cartItem.reduce((total,cartItem)=> total + cartItem.price * cartItem.quantity,0)
        ...
        {cartItem.map((item) => (
          <div className="flex justify-between px-2">
            ...
            <span className="inline-flex items-center gap-2">
              <img onClick={()=> subFromCart(item)} src="/remove.png" className="w-4 cursor-pointer active:shadow-xl" />
              <div> {(item.price / 100).toFixed(2)} </div>
              <img onClick={()=> removeCart(item)}  src="/trash.png" className="w-4 cursor-pointer active:shadow-xl " />
            </span>
